...
To find all packages depending on a package \code{<pkg>}, we can search all available package.json files for the substring \code{"<pkg>":}.
...
\TODO{Is this the right place to introduce TypeScript?}
...
This visitor function matches each node against a list of defined patterns (similar to \cref{fig:usage_mining/patterns}) based on the \code{syntaxKind}s of the node and its ancestors and descendants and then looks up the declaration of the node's type symbol to compare it with the source directory of the target package.
...
\begin{figure*}[b]
	...
	\begin{subfigure}{.32\linewidth}
		\includegraphics[width=\textwidth]{sections/6_implementation/extension/dependencies.png}
...
As the VSCE API per se does not provide for multithreading or multiprocessing operations but suggests a Promise/A+-driven dataflow, we adopt this style for our data model using the JavaScript concepts asynchronous functions, \code{AsyncIterator}s, and \code{Promise.all} to avoid busy waiting inside the data collection process.
