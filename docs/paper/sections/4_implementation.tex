...
npm packages specify their dependencies in a \code{package.json} metadata file (see \cref{sec:implementation/package.json}).
...
As the VSCE API per se does not provide for multithreading or multiprocessing operations but suggests a Promise/A+-driven dataflow, we adopt this style for our object model by using the JavaScript concepts asynchronous functions, \code{AsyncIterator}s, and \code{Promise.all} to avoid busy waiting during the data collection process.
